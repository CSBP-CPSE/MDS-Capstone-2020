...
data <- data.frame(data[1:3], lapply(data[4:24], prop), data[25:30])
```


Keep only the predictors and response variables needed for the analysis
```{r}
final_cases <- subset(data, select = -c(Location_x, FATAL, TOTAL, HRPOP, FATAL_prop, TOTAL_prop, Fatal_prop))
final_fatalities <- subset(data, select = -c(Location_x, FATAL, TOTAL, HRPOP,Total_prop, TOTAL_prop, FATAL_prop))
```
...
```{r}
library(betareg)
beta_model <- betareg(Total_prop~Arthritis..15.years.and.over.+Body.mass.index..adjusted.self.reported..adult..18.years.and.over...obese+Heavy.drinking+amenity_dense,data = final_cases, link = "loglog")
summary(beta_model)
...
ggplot(final_cases, aes(x=Heavy.drinking, y=Total_prop, col = amenity_dense)) +
  theme_classic()+
  geom_point(alpha = 0.9, aes(colour=amenity_dense), size=4) + 
...
ggrepel::geom_label_repel(data=data %>% filter(Heavy.drinking == 0.198), aes(label = Location_x, fill=factor(amenity_dense)),label.size = 0.3, color = "black",  nudge_x = 0.055, nudge_y = 0.0015)+
  
geom_label( data=data %>% filter(Total_prop> 0.004),aes(label=Location_x, fill = factor(amenity_dense)), color = "black",label.size = 0.3, label.padding = unit(0.15, "lines"), nudge_x = 0.028, nudge_y = 0.00015)+ 
...
beta_model2 <- betareg(Fatal_prop~Body.mass.index..adjusted.self.reported..adult..18.years.and.over...obese+Chronic.obstructive.pulmonary.disease..COPD..35.years.and.over.+Heavy.drinking+Sense.of.belonging.to.local.community..somewhat.strong.or.very.strong+amenity_dense,data = final_fatalities, link = "loglog")
summary(beta_model2)
...
ggplot(final_fatalities, aes(x=Sense.of.belonging.to.local.community..somewhat.strong.or.very.strong, y=Fatal_prop, col = amenity_dense)) +
  theme_classic()+
  geom_point(alpha = 0.9, aes(colour=amenity_dense), size=4) + 
...
#geom_label_repel( data=data %>% filter(Fatal_prop > 0.00035),aes(label=Location_x, fill = factor(amenity_dense)), color = "black",label.size = 0.3, label.padding = unit(0.15, "lines"),nudge_x = 3, nudge_y = 0.5)+ 
